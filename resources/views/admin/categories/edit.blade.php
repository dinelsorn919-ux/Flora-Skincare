...
@section('content')
    <div class="max-w-xl mx-auto px-6 py-12">
        <div class="flex items-center justify-between mb-8">
            <div>
                <h1 class="font-display text-3xl text-ink mb-1">Edit Category</h1>
                <p class="text-xs text-ink/60">Update details for {{ $category->name }}</p>
            </div>
        ...
        <div class="bg-white border border-line rounded-3xl p-8 lg:p-10 shadow-xl relative overflow-hidden">
            <form action="{{ route('admin.categories.update', $category) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input id="name" type="text" name="name" value="{{ old('name', $category->name) }}" required placeholder="e.g. Skincare"
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3" placeholder="Brief category summary..."
                        class="w-full border border-line rounded-xl px-4 py-2.5 text-xs bg-bone/30 focus:outline-none focus:border-forest focus:bg-white transition-all">{{ old('description', $category->description) }}</textarea>
                </div>
